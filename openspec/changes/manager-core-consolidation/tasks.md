...
- [x] 1.8 验收：`openspec validate --all --strict`、`pnpm test -- --runInBand`、`pnpm typecheck`、`pnpm --dir webui check`、`pnpm build`、`pnpm exec vp fmt --check`、`git diff --check`。（2026-09-06 实测：tests 212/212、typecheck 0 错、svelte-check 0 错 0 警、build 成功、fmt 全部合规、diff --check 干净、openspec 4/4 通过、npm pack --dry-run 70 文件正常）
