...
- [x] 1.4 完成 Creator new/edit：draft 只在组件内，save/delete 携带 target 和 expectedRevision；冲突不覆盖 draft，重载后用户能继续决定。（file-browser 补 Delete：confirm + busy 锁 + expectedRevision；CONFLICT 不覆盖 draft，toast 提供显式 Reload；removeSkill 签名改为显式 target+skillId+revision；草稿组件级 $state 机制保持不变；svelte-check 0 错）
- [x] 1.5 完成 Repository：source list/add/remove、scan commit、remote skill selection、preview、dry-run、multi-target install；session 失效时提示重新扫描。（新增 header ref 输入（branch/tag）+ Rescan；repository store 返回结构化 RepositoryCallFailure，`isSessionExpired` 判定 UNAVAILABLE+session expired，失效时显示专门横幅 + Rescan（不再裸渲染错误串）；修复 viewInWorkspaces 传入 schema 不存在的 highlight 参数；svelte-check 0 错）
- [x] 1.6 为所有 mutation 加 disabled/loading 锁和结果分类；禁止 skipped-only 显示为成功 0 项。（toggle/save/delete/validate/install/add-source/import/remove-workspace 全部有 busy 锁；remove-source 从 native confirm + console.error 升级为 ConfirmDialog + busy 锁 + toast 终态；install 汇总四分类计数与 apply 汇总三分类计数均由逐项状态重算，skipped-only 显示为 skipped 计数而非成功）
