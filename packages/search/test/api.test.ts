/**
 * @jixoai/search API 语义测试（backend-agnostic，本阶段跑 sqlite 后端；
 * tantivy 接入后同一组测试复用——tasks 1.10）。
 *
 * User input [2026-09-21]：「两后端仅召回实现不同，命中与分数逐位一致；
 * 幂等 upsert/分页/稳定排序/stored 回传不打分/remove/信封不匹配自动重建/
 * fuzzy-prefix/中英混合均为包级单测。」（jixoai-search-core 1.10。）
 *
 * Orthogonal intents:
...
import { openIndex, SearchError, type SearchIndex } from "../src/index.js";
...
async function openTestIndex(): Promise<SearchIndex> {
  index = await openIndex({ directory: indexDirectory(), fields: FIELDS });
  return index;
}

describe("search index api semantics (sqlite backend)", () => {
  it("upserts idempotently: same id overwrites instead of duplicating", async () => {
    const search = await openTestIndex();
    await search.upsert([
      {
        id: "a",
  ...
  it("rebuilds an empty index when the envelope no longer matches", async () => {
    const first = await openTestIndex();
    ...
    const second = await openTestIndex();
    index = second;
  ...
  it("keeps the tantivy backend as a typed unavailable placeholder", async () => {
    await expect(
      openIndex({ directory: indexDirectory(), fields: FIELDS, backend: "tantivy" }),
