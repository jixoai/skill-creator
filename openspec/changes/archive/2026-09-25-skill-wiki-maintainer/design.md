...
> r21（2026-09-25，实现复核 r3 处置）：merge 信封校验与页面序列化是纯
> 计算，前置于 onIntent 预留（恢复回填与首放去重两处同款）——坏信封
> typed 拒绝不消耗写页预算，重放不因永久坏信封耗尽 attempts。
> r22（2026-09-25，实现复核 r4 处置）：absorb 执行路径同款前移（edits
> 试算 + merge + 序列化先于 onIntent）——补齐 r21 未覆盖的相邻路径；
> 附带裁决：patch-failed 是确定性终态、从未写页，不再消耗写页预算
> （原「intent 先行」语义作废，回归同步更新）。
