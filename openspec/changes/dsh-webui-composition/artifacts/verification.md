...
## 4.1 实测注记（生产 daemon 默认 DSH 组合宿主 + release evidence）

- daemon 默认入口切换：`src/daemon/dsh-host-lifecycle.ts` `mountProductionDshHost`（boot 官方 profile → `WebServer.mountDsh` 同源挂载；失败显式卸载回 SPA 恢复夹具 + `status.dsh` 携带 reason；env `SKILL_CREATOR_DSH_HOST=off` 逃生口）；`bootDaemon` web 就绪后挂载、`performStop` 有界回收（先于 web server teardown）。`DaemonHandles` 增补 `domain`/`dshHost`（进程内组合/取证面；跨进程消费者仍走 RPC/IPC）。
- boot graph（真实激活事实）：`bootOfficialWebProfile` 的 prepare 钩子在 tree mount 前订阅 `loader/entry-init`（事件期 Entry 构造器尚未赋 options/parent，只收集对象引用；settle 后读 `loader.entries()` 与 options.name）→ 149 entries + 等长 activationOrder（含 `@skill-creator/dsh-client` 行）。`status.dsh` 暴露 `{mounted, reason?, port, entries[], activationOrder[]}`（token 绝不进 status）。
- 生命周期矩阵（`pnpm exec tsx scripts/dsh-release-evidence.sh.ts`，artifacts/dsh-release-evidence.json）：组合启动（boot graph + `/api/health` + `/manager/dsh-island.js` 同源 + `/?token=` 303 握手 + `__DSH_BOOT__` 注入）→ Manager 原有操作 fs diff（daemon 同一 domain：import workspace + create skill 精确 +1 `skills/release-evidence-skill/SKILL.md`（sha256 记录）；disable → `SKILL.md`→`.SKILL.md`、enable 恢复）→ 真实 tool round（daemon 内 binder + pipeline，dshSession + 2 toolCalls + 终态叙述）→ 仅卸载宿主（SPA 恢复、daemon 存活、`__DSH_BOOT__` 消失）→ 全量 stop（endpoint ECONNREFUSED）→ 同 home 重启（重新挂载 + 握手 303）。
- 降级矩阵（每例独立 home）：`unusable-home`（ENOTDIR typed reason + SPA 200）、`plugin-linkage-failure`（断插件链接 EEXIST typed reason + SPA 200）、`core-module-heal-repair`（profile node_modules 核心包断链被 heal 闭包镜像按设计修复并正常挂载——记录为恢复事实，非降级）、`env-off`（disabled by env + SPA 200）。版本漂移声明：repo 级缺包/错版无法不破坏工作树注入，同走模块解析失败 fail-closed 路径；完整版本矩阵归阶段 5 生产 pack clean-install（tasks 4.1 明示）。
- 浏览器取证（`--hold` 模式，9 张截图 dsh-web-4.1-*.png，全程 0 JS 错误）：组合入口（sidebar 绑定 Steward 会话 + footer Manager 入口 + 模型/预设/访问模式）、session transcript（「2 次工具调用」+「1 轮 · 1 步」+ 终态）、tool round 展开行（`skills.list_context · {}`）、settings（通用/模型/插件/Agent 预设/权限）与 models tab、Manager island（真实 catalog 543 skills/75 locations + 导入 ws）→ skill 详情 → UI Disable/Enable（磁盘 `SKILL.md`↔`.SKILL.md` 前后 ls 证据）；`--hold --degraded` 恢复夹具（Manager-only：无 Agent/session/chat/transcript 面、`window.__DSH_BOOT__` 不存在）。
