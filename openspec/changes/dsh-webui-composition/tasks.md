...
- [x] 4.1 完成浏览器、daemon、filesystem 和 release evidence。
  ...
  - Evidence: daemon 默认入口切换 `src/daemon/dsh-host-lifecycle.ts`（mountProductionDshHost：成功同源挂载/失败显式降级 SPA + status.dsh reason/env off；performStop 有界回收）+ `bootOfficialWebProfile` 真实 boot graph（prepare 订阅 loader/entry-init 构造序 + settle 后 loader tree：149 entries 含 Manager plugin 行；status.dsh 暴露 port/entries/activationOrder）+ `test/dsh-host-lifecycle.test.ts` 3/3。`scripts/dsh-release-evidence.sh.ts`（artifacts/dsh-release-evidence.json）：组合启动（health/island 资产/303 握手/**DSH_BOOT** 注入）→ Manager fs diff（create 精确 +1 SKILL.md+sha256；disable/enable `SKILL.md`↔`.SKILL.md`）→ 真实 tool round（daemon 内 session 绑定 + 2 toolCalls）→ 仅卸载宿主 SPA 恢复（daemon 存活）→ stop endpoint 释放 → 同 home 重启再挂载；降级矩阵 unusable-home/plugin-linkage-failure/env-off fail-closed + core-module-heal-repair（heal 按设计修复，记录为恢复事实）；版本漂移矩阵归阶段 5 pack clean-install。浏览器（--hold，9 张 dsh-web-4.1-*.png，0 JS 错误）：入口/绑定会话 transcript（2 次工具调用+终态）/tool 行展开/settings+models/Manager island（真实 catalog + 导入 ws）→ skill 详情 → UI Disable/Enable 磁盘证据；--hold --degraded 恢复夹具 Manager-only（无 Agent/session/chat、无 **DSH_BOOT**）。轮内真实缺陷修复：binder tool/call `arguments` 改官方契约 JSON 字符串（对象值使官方 GenericToolCard 渲染崩溃；test/dsh-tool-composition.test.ts 契约断言）。门禁：440/440（53 files）、typecheck 0、webui check 0/0、build 通过、fmt 468 clean、diff-check clean、openspec 9/9。

- [ ] 4.2 入口握手桥：真人浏览器从 daemon origin 单入口直达 DSH 融合 shell。
  - Files: `src/daemon/web-server.ts`, `src/daemon/dsh-host-lifecycle.ts`, `test/dsh-manager-mount.test.ts`。
  - Steps: DSH 挂载时裸 `/`（无 cookie、无 token）303 到同源 `/?token=…`（进程级 launch token 的 path+search）；DSH 设 `dsh-auth-<hash(authority)>` 会话 cookie 后 303 回干净 `/`；fragment 由浏览器保留供 island hash-capture 取 daemon web token；cookie 名正确但值失效时代理把 401 转 303 自愈；DSH 未挂载时 SPA 恢复面不变。
  - Acceptance: 真人浏览器打开 daemon webUrl 即落在 DSH shell（无手动 token 操作、无 401）；sidebar 的 Manager island 可用；防环有测试钉（带 cookie 的 `/` 直接 200；带 token 的 401 如实呈现）；生产 tray 窗口同路径实测。
