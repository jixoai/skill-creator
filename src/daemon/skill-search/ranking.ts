 ...
 * 修订 [2026-09-18]（用户真实语料走查）：「跨 agent 的同内容副本动辄 30+ 份；top-40
 * 池被副本挤爆导致结果只剩一两组，且折叠代表只带自身 installations，按 provider
 * 作用域过滤的 GUI 消费方永远空。」→ v2：折叠前移到 top-40 池之前（组代表进池），
 * primary 的 installations 合并组内全部成员入口。
 * 正交意图：
 * 1. 冻结 rerank 信号集（大小写不敏感 + query trim，上限 1.0）与混合归一。
 * 2. 池前 content-dup 折叠（组代表 = bm25 desc → canonicalPath asc）+ primary
 *    installations 合并去重（primary 先、组内序追加）。
...
/** 排序规则版本；任何公式/权重/折叠策略变化必须递增并触发索引全量重建。 */
export const RANKING_VERSION = "rerank-2026-09-18-v2";
...
/**
 * 冻结排序管线（v2）：全量候选按 contentHash 分组 → 组代表（bm25 desc →
 * canonicalPath asc）进入 top-40 池 → 逐代表 rerank → final → tie-break →
 * top limit。primary 的 installations 合并组内全部成员入口（primary 先、组内
 * 冻结序追加，按 path+作用域去重）；组内其余成员进 duplicates（同冻结序）。
 * 输入次序之外的任何扫描顺序都不影响输出。
 */
export function rankResults(
  ...
  const groups = new Map<string, RankingCandidate[]>();
  for (const candidate of candidates) {
    const group = groups.get(candidate.contentHash) ?? [];
    group.push(candidate);
    groups.set(candidate.contentHash, group);
  }

  // 池前折叠：每组成员按冻结组内序（bm25 desc → canonicalPath asc）整理，代表进池。
  const orderedGroups = [...groups.values()].map((group) => group.slice().sort(byGroupOrder));
  const pool = orderedGroups
    .map((group) => group[0])
    .sort(byGroupOrder)
  ...
  scored.sort(byFrozenTieBreak);

  return scored.slice(0, limit).map(({ candidate, final }) => {
    const group = groups.get(candidate.contentHash) ?? [candidate];
    const members = group.filter((member) => member !== candidate).sort(byGroupOrder);
    return {
      id: candidate.id,
      name: candidate.name,
      description: candidate.description,
      canonicalPath: candidate.canonicalPath,
      installations: mergeInstallations(candidate, members),
      contentHash: candidate.contentHash,
      disabled: candidate.disabled,
      conflict: candidate.conflict,
      score: final,
      duplicates: members.map((member) => ({
        id: member.id,
        canonicalPath: member.canonicalPath,
      })) satisfies SkillSearchDuplicate[],
    };
  });
}

/** 组内冻结序：bm25 desc → canonicalPath asc（codepoint 序）。 */
function byGroupOrder(left: RankingCandidate, right: RankingCandidate): number {
  if (left.bm25 !== right.bm25) return right.bm25 - left.bm25;
...
/** primary installations 在前，其余成员按组内冻结序追加；按 path+作用域去重。 */
function mergeInstallations(
  primary: RankingCandidate,
  members: readonly RankingCandidate[],
): SkillSearchResult["installations"] {
  const seen = new Set<string>();
  const merged: SkillSearchResult["installations"] = [];
  for (const installation of [primary, ...members].flatMap(
    (candidate) => candidate.installations,
  )) {
    const key = `${installation.path}|${installation.workspaceId}|${installation.providerId}`;
