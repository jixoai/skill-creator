...
- [ ] 1.5 冻结 tokenizer 下沉（从 src/daemon/skill-search/tokenizer.ts
       迁入包内，TOKENIZER_VERSION 延续；daemon 侧 re-export 过渡）
       + 冻结期望表平移
- [ ] 1.6 通用 API 契约层：文档/命中/字段声明类型 + openIndex/upsert/
...
- [ ] 3.1 parseWikiScope 放宽为 npm-scope 式 slug（破坏性，含测试更新）
       + 宿主 workspaceId → slug 映射层（wiki-service 适配）
