...
- [ ] 1.2 将 Skill Creator Manager 注册为 DSH client plugin。
  ...
  - Steps: 提供 `client` export 和 `dsh.client` manifest，声明 inject/external；通过 DSH slots/remote 接入 Workspace/Provider/Skill/Steward surfaces；mount/dispose 由 DSH lifecycle 管理。
  - Acceptance: plugin 只建立一个 Manager RPC owner；无第二个 SvelteKit shell、无 iframe、无 ACP chat route；plugin activation failure 阻止假成功启动。
